...
const API = ''  // mismo origen en producción
...
export default function App() {
  ...
  useEffect(() => {
    api('/auth/me').then(u => { setUser(u); loadRespawns() }).catch(() => {})
  }, [api, loadRespawns])
  ...
  // Calcular claim activo del usuario
  ...
  if (!user) return (
    <div style={{ background: '#0f0f13', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
      <div style={{ background: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: 10, padding: 32, width: 320 }}>
        <div style={{ fontFamily: "'Cinzel',serif", color: '#c9a84c', fontSize: 22, textAlign: 'center', marginBottom: 4 }}>⚔ Dark Brotherhood</div>
    ...
    <div style={{ background: '#0f0f13', minHeight: '100vh', fontFamily: 'sans-serif', color: '#e0ddd5' }}>
      ...
      <div style={{ background: '#12121a', borderBottom: '1px solid #2a2a3a', padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontFamily: "'Cinzel',serif", color: '#c9a84c', fontSize: 17, fontWeight: 600 }}>Dark Brotherhood</div>
                  ...
                  <div key={r.id} style={{ background: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: 8, padding: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                      ...
                      <span style={{
                        ...
                        {r.maintenance ? 'Mantenimiento' : isHolder ? 'Tu respawn' : !c ? 'Libre' : 'Ocupado'}
                      </span>
                    </div>

                    {r.maintenance && <div style={{ fontSize: 12, color: '#555', textAlign: 'center', padding: '12px 0' }}>No disponible temporalmente</div>}
                    ...
                    {!r.maintenance && c && (
                      <>
                        ...
                        {!isHolder && !isNext && c.next_name && !isNext && (
        ...
        {tab === 'admin' && user.role === 'admin' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <tbody>
                  {respawns.map(r => (
                    <tr key={r.id}>
                      ...
                      <td style={{ padding: '6px 8px' }}>{r.claim ? <span style={{ color: '#e05252' }}>🗡 {r.claim.holder_name}</span> : <span style={{ color: '#4caf7a' }}>Libre</span>} {r.maintenance && <span style={{ color: '#777', marginLeft: 4 }}>[Mant.]</span>}</td>
                      <td style={{ padding: '6px 8px', display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        ...
                        <button onClick={async () => { await api(`/admin/respawns/${r.id}`, { method: 'PUT', body: { maintenance: !r.maintenance } }); loadRespawns(); loadAdminData() }} style={{ background: '#1a1a2a', border: '1px solid #3a3a4a', color: '#aaa', padding: '2px 8px', borderRadius: 4, cursor: 'pointer', fontSize: 11 }}>{r.maintenance ? '✓ Reactivar' : '🔧 Mant.'}</button>
                        <button onClick={async () => { try { await api(`/admin/respawns/${r.id}`, { method: 'DELETE' }); loadRespawns(); loadAdminData(); toast('🗑 Eliminado') } catch (e) { toast(e.message, true) } }} style={{ background: '#2a2a2a', border: '1px solid #3a3a4a', color: '#666', padding: '2px 8px', borderRadius: 4, cursor: 'pointer', fontSize: 11 }}>🗑</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div style={{ background: '#12121a', border: '1px solid #2a2a3a', borderRadius: 8, padding: 14 }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead><tr>{['Personaje', 'Rol', 'Estado'].map(h => <th key={h} style={{ textAlign: 'left', color: '#888', padding: '6px 8px', borderBottom: '1px solid #2a2a3a', fontWeight: 500 }}>{h}</th>)}</tr></thead>
                <tbody>
                  {adminUsers.map(u => (
                    <tr key={u.id}>
                      <td style={{ padding: '6px 8px', color: '#e8e4d8' }}>{u.username}</td>
                      <td style={{ padding: '6px 8px' }}><span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 10, background: u.role === 'admin' ? '#0d1a3a' : '#0d3320', color: u.role === 'admin' ? '#4a90d9' : '#4caf7a', border: `1px solid ${u.role === 'admin' ? '#1a3a6a' : '#1d5535'}` }}>{u.role}</span></td>
